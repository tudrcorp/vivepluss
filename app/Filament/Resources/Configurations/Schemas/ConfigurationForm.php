...
{
    /**
     * Valida, en el momento de la subida, que el PDF de portada/contraportada sea
     * procesable por FPDI y tenga exactamente 1 página (es la plantilla de una sola
     * página que luego se combina con el resto del documento generado dinámicamente).
     */
    private static function singlePagePdfRule(): \Closure
    {
        return function (string $attribute, $value, \Closure $fail) {
            ...
            try {
                $fpdi = new Fpdi;
                $pageCount = $fpdi->setSourceFile($value->getRealPath());
            } catch (\Throwable $e) {
                $fail('El PDF no se pudo procesar. Si fue exportado desde Illustrator, Word o Canva, intenta guardarlo como "PDF 1.4" o sin compresión de referencias cruzadas y vuelve a intentar.');

                return;
            }

            if ($pageCount !== 1) {
                $fail("El PDF debe tener exactamente una página (tiene {$pageCount}).");
            }
        };
    }
    ...
    public static function configure(Schema $schema): Schema
    {
        return $schema
            ->components([
                Tabs::make('ConfigurationTabs')
                    ->tabs([
                        Tab::make('Configuración del Sistema')
                            ->icon('heroicon-o-cog')
                            ->schema([
                                ...self::systemSchema(),
                                ...self::systemTablesSchema(),
                            ]),
                        Tab::make('Configuración de la Página Web')
                            ->icon('heroicon-o-globe-alt')
                            ->schema(self::webSchema()),
                    ])
                    ->columnSpanFull(),
            ]);
    }

    private static function systemSchema(): array
    {
        return [
            ...
            Fieldset::make('Portada y Contraportada — Cotización Individual')
                ->schema([
                    Grid::make(2)->schema([
                        FileUpload::make('quote_cover_individual')
                            ->label('Portada (página 1)')
                            ->disk('public')
                            ->directory('quote-templates')
                            ->acceptedFileTypes(['application/pdf'])
                            ->rules([fn () => self::singlePagePdfRule()])
                            ->helperText('PDF de una sola página. Su tamaño de página define el tamaño final de todo el documento. Si no se configura, la cotización se genera con el diseño estándar actual.'),
                        FileUpload::make('quote_back_cover_individual')
                            ->label('Contraportada (página 3)')
                            ->disk('public')
                            ->directory('quote-templates')
                            ->acceptedFileTypes(['application/pdf'])
                            ->rules([fn () => self::singlePagePdfRule()])
                            ->helperText('PDF de una sola página. Si no se configura, la cotización se genera con el diseño estándar actual.'),
                    ])->columnSpanFull(),
                ])->columnSpanFull(),
    ...
    private static function webSchema(): array
    {
        return [
    ...
    private static function systemTablesSchema(): array
    {
        return [
            ...
            Fieldset::make('Notificaciones de Documentos (Integracorp)')
                ->schema([
                    ToggleButtons::make('document_notifications_enabled')
                        ->label('¿Notificar cuando llegue un certificado o carnet nuevo?')
                        ->boolean()
                        ->inline()
                        ->live()
                        ->default(false)
                        ->helperText('Se dispara cada vez que Integracorp entrega un documento vía webhook (uno por certificado, uno por cada carnet). Si está desactivado, no se envía ninguna notificación.'),
                    TagsInput::make('document_notification_emails')
                        ->label('Correos a notificar')
                        ->helperText('Presiona Enter después de cada correo. Reciben un email con el PDF adjunto.')
                        ->placeholder('correo@ejemplo.com')
                        ->visible(fn (Get $get) => (bool) $get('document_notifications_enabled'))
                        ->columnSpanFull(),
                    TagsInput::make('document_notification_phones')
