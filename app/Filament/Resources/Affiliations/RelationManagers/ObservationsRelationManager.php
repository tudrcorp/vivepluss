...
{
    protected static string $relationship = 'affiliationObservations';
    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->description('Bitácora de notas internas registradas sobre esta afiliación, ordenadas de forma cronológica.')
            ...
            ->columns([
                TextColumn::make('created_at')
                    ->label('Fecha')
                    ->dateTime('d/m/Y H:i')
                    ->description(fn (AffiliationObservation $record): string => $record->created_at?->diffForHumans() ?? '')
                    ->sortable(),
                ...
                TextColumn::make('created_by')
                    ...
                    ->formatStateUsing(fn (AffiliationObservation $record): string => $record->createdBy?->name ?? (string) ($record->created_by ?? '—'))
                    ->description(fn (AffiliationObservation $record): ?string => $record->createdBy?->email),
            ])
            ->headerActions([
                Action::make('addObservation')
                    ...
                    ->action(function (array $data): void {
                        InternalObservations::store($this->getOwnerRecord(), 'affiliationObservations', $data);
                    }),
            ])
            ->paginated([10, 25, 50]);
    }
