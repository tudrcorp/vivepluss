                        ...
                        ->schema([
                            ...
                            Grid::make(3)->schema([
                                ...
                                Select::make('coverage_id')
                                    ...
                                    ->disabled(function () {
                                        $coverageId = request()->query('coverage_id');
                                        if (isset($coverageId) && $coverageId != null) {
                                            return true;
                                        }
                                    ...
                                    ->preload(),
                                Hidden::make('detail_ids')
                                    ->default(fn () => request()->query('detail_ids'))
                                    ->dehydrated(false),
                                Select::make('payment_frequency')
                                    ...
                                    ->afterStateUpdated(function ($state, $set, Get $get) {
                                        /**
                                         * IDs de las filas de detail_individual_quotes (una por rango de
                                         * edad) que el analista realmente seleccionó en "Preparar
                                         * afiliación". Sin este filtro, una cotización con varios rangos
                                         * de edad para el mismo plan+cobertura (p.ej. 0 a 45, 46 a 75,
                                         * 76 a 85) sumaba la tarifa de TODOS los rangos aunque solo se
                                         * hubiera seleccionado uno.
                                         */
                                        $detailIds = collect(explode(',', (string) $get('detail_ids')))
                                            ->filter()
                                            ->map(fn ($id) => (int) $id);
                                        ...
                                        $set('total_amount', match ($get('payment_frequency')) {
                                            ...
                                            'TRIMESTRAL' => $quoteQuery('subtotal_quarterly'),
                                            'SEMESTRAL' => $quoteQuery('subtotal_biannual'),
                                            'MENSUAL' => $quoteQuery('subtotal_monthly'),
                                            default => $get('total_amount'),
                                        });

                                        $set('fee_anual', $quoteQuery('subtotal_anual'));
                                    }),
